...
load data local infile './data/icd_annotation.txt' replace into table icd_annotation
...
load data local infile './data/icd_category.txt' replace into table icd_category
	(name, display, sorting_label, definition)
	set instance_name=concat(@instance, name), instance=@instance;
load data local infile './data/icd_category_children.txt' replace into table icd_category_children
...
load data local infile './data/icd_change.txt' replace into table icd_change
...
load data local infile './data/icd_linearizationspec.txt' replace into table icd_linearizationspec
...
load data local infile './data/icd_ontologycomponent.txt' replace into table icd_ontologycomponent
	(@name, type, @category_id)
	set annotatablething_ptr_id=concat(@instance, @name), _instance=@instance, _name=@name,
	category_id=concat(@instance, @category_id);
load data local infile './data/icd_user.txt' replace into table icd_user
	(type, name)
	set instance_name=concat(@instance, name), instance=@instance;
load data local infile './data/icd_user_domain_of_interest.txt' replace into table icd_user_domain_of_interest
...
load data local infile './data/icd_user_watched_branches.txt' replace into table icd_user_watched_branches
	(@user_id, @ontologycomponent_id)
	set user_id=concat(@instance, @user_id), ontologycomponent_id=concat(@instance, @ontologycomponent_id);
load data local infile './data/icd_user_watched_entities.txt' replace into table icd_user_watched_entities
	(@user_id, @ontologycomponent_id)
	set user_id=concat(@instance, @user_id), ontologycomponent_id=concat(@instance, @ontologycomponent_id);
